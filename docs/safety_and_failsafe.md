...
- 카메라에 보이는 임의의 수동 배치 차량을 `VISION_OCCUPIED`로 분류해 allocator와 자동 동기화하는 production 경로는 없다.
- 장애물 geometry는 정상 `PARKED`로 확정된 차량 또는 fresh trusted heading이 있는 다른 차량에 대해 적용된다. heading이 불확실한 bound 차량은 새 route를 fail-safe로 막는다.
