...
현재 camera-only 정적 차량을 자동으로 슬롯 `VISION_OCCUPIED` 상태로 만드는 경로는
없다. 슬롯 제외가 필요하면 allocator가 사용하는 slot state에 실제 예약/점유 상태가
들어가야 한다. 이미 binding된 다른 차량의 fresh trusted Pose는 trajectory obstacle로
사용되며, heading이 불확실하면 새 route를 fail-safe로 막는다. 이 기능을
`VISION_OCCUPIED` 자동 검증 완료로 표현하면 안 된다.
